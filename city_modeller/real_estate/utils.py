...
def estimate_uva_pct_growth(
        permission_month: int, 
        permission_year: int, 
        uva_last_avbl_date: pd.Timestamp
    ) -> float:
    """
    ...
    uva_historic_cumsum = uva_historic_vals.loc[
        uva_historic_vals.date >= past_date,'value'
        ].pct_change().sum()
...
def update_urban_land_incidence(
        parcel_uva: float, 
        pct_growth:float
    ) -> float:
    """

...
def loads_kepler_geomstr(str_geom: dict[str, str]) -> dict:
...
def kepler_geomstr_to_gdf(json_polygon: dict, crs_code: str | int) -> gpd.GeoDataFrame:
    polygon_geom = Polygon(json_polygon["coordinates"][0])
    gdf = gpd.GeoDataFrame(index=[0], crs="epsg:4326", geometry=[polygon_geom]).to_crs(crs_code)
    return gdf


def read_kepler_geometry(geom: dict[str, str]) -> Union[BaseGeometry, None]:
    try:
...
def transform_kepler_geomstr(str_geometry: str, crs_code: str | int) -> gpd.GeoDataFrame:
...
def show_calendar(
        legend: str,
        last_avbl_datetime: pd.Timestamp, 
        month_abbr: calendar._localized_month
    ) -> tuple[int, int]:
    with st.expander(legend): 
        ...
        permission_month_abbr = st.radio("", month_abbr, index=last_avbl_month - 1, horizontal=True)
